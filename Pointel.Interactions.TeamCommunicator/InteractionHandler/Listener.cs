            ...
            try
            {
                ...
                if (CMEObjects != null)
                {
                    ...
                    if (CMEObjects.ContainsKey("teamcommunicator.custom-favorite-list"))
                    {
                        ...
                        foreach (string item in tempList)
                            _dataContext.CustomFavoriteList.Add(item.TrimStart().TrimEnd());
                    }
                    if (CMEObjects.ContainsKey("teamcommunicator.internal-favorite-list"))
                    {
                        ...
                        foreach (string item in tempList)
                            _dataContext.InternalFavoriteList.Add(item.TrimStart().TrimEnd());
                    }
                    if (CMEObjects.ContainsKey("teamcommunicator.max-recent-records") && Convert.ToInt32(CMEObjects["teamcommunicator.max-recent-records"]) > 0)
                    {
                        _dataContext.RecentMaxRecords = Convert.ToInt32(CMEObjects["teamcommunicator.max-recent-records"]);
                    }
                    if (CMEObjects.ContainsKey("teamcommunicator.max-suggestion-size") && Convert.ToInt32(CMEObjects["teamcommunicator.max-suggestion-size"]) > 0)
                    {
                        _dataContext.MaxSuggestionSize = Convert.ToInt32(CMEObjects["teamcommunicator.max-suggestion-size"]);
                    }
